...
function fillModal(recipeid, jsonObject) {
  ...
  let htmlString = `<h1>${recipe.name}</h1><p>Tags: ${recipe.tags.join(
    ...
    recipe.name
  }" /><br><label for="multiplier">Ingredient multiplier: </label><select id="select-multiplier" name="multiplier"><option value="1" selected>1x</option><option value="1.5">1.5x</option><option value="2">2x</option><option value="3">3x</option><option value="4">4x</option><option value="5">5x</option></select><h2>Ingredients</h2><ul id="ingredients-list">`;
